...
function categorizeData () {
    return new Promise(function(resolve, reject) {
        ...
        for (let i = 0; i < categories.length; i++) {
            ...
            // convert totalSpend to show as currency
            categories[i].totalSpend = '$' + (categories[i].totalSpend).toFixed(2);
...
// Page #1: Render homepage - list of transactions
function homePage () {
    app.get('/', (req, res) => {
        res.status(200);
        res.render('index', {
...
// Page #2: Render categorization page - spending broken up by category
function categorizationPage () {
    app.get('/categorization', (req, res) => {
        res.status(200);
        res.render('categorization', {
