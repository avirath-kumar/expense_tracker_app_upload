...
// Connect to MongoDB
const dbURI = 'mongodb+srv://avikumar:LoathingMaulDivider7878@cluster0.frjes3s.mongodb.net/expense_tracker_app?retryWrites=true&w=majority';
const csvFilePath = './transactions/01.10.23-02.06.23.csv'

function connectToMongo () {
    return new Promise (function(resolve, reject) {
        mongoose.connect(dbURI, { useNewUrlParser: true, useUnifiedTopology: true })
            .then((result) => console.log('succesfully connected to MongoDB'))
...
function csvToDB () {
    return new Promise (function(resolve, reject) {
        csv().fromFile(csvFilePath)
            .then((result) => {
                const jsonArray = JSON.stringify(result, null, 2);
                // console.log(jsonString);
                Transaction.insertMany(jsonArray);
